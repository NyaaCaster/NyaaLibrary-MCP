...
├── docker-compose.yml      # 单服务编排（对外端口 5106→容器 5101，挂载 ./data）
...
| 服务 | 容器名            | 端口      | 镜像                   |
| ---- | ----------------- | --------- | ---------------------- |
| app  | nyaalibrary-mcp   | 5106:5101 | nyaalibrary-mcp:latest |
...
访问 `http://localhost:5106` 进入控制台。

**数据持久化**：SQLite 数据库与向量库存放于容器内 `/app/data`，通过 `docker-compose.yml`
绑定挂载到项目目录 `./data`。容器重启或重建后数据自动恢复；删除 `./data` 即重置全部知识库。

> 首次构建会编译 `better-sqlite3` 原生模块；在 Windows Docker Desktop 上前端依赖
> 安装（含 `lucide-react` 大量图标文件）可能较慢，属正常现象，后续构建走缓存即可。
...
见 [`.docs/BLUEPRINT.md`](.docs/BLUEPRINT.md)。核心里程碑 **M0–M4 全部完成**：基础骨架、
文档摄取管线（解析 .txt·.md·.pdf·.docx·.epub·.xls·.xlsx / 分块 / 嵌入 / 稠密+稀疏混合检索）、
文档管理 UI、检索测试与配置、响应式打磨与 Docker 部署验证（端口 5101、`./data` 持久化）。
